...
isTruthy([]);  //true
isTruthy(-1);  //true
isTruthy(function(){return 0});  //true
isTruthy([{}, {}, {}]);  //true
isTruthy(0.125);  //true
...
carro.addPessoas = function(x){
  var a = (carro.quantidadePessoas + x);
  
  if(carro.quantidadePessoas === carro.assentos && a >= carro.assentos){
    "O carro já está lotado!"
  }
  if(a > carro.assentos){
    var b = carro.assentos - carro.quantidadePessoas;
    var pessoa = a === 1 ? ' pessoa' : ' pessoas';
    return "Só cabem mais " + b + pessoa + "!"
  }
...
carro.obterMarcaModelo(); //'Esse carro é um Tesla Model 3'
